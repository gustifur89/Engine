...
void main()
{
	vec3 fragPos   = (viewMatrix * vec4(texture(posTex, UV).xyz,1)).xyz;
	vec3 normal    = (viewMatrix * vec4(texture(normTex, UV).xyz, 0)).xyz;
	...
	for(int i = 0; i < kernelSize; ++i)
	{
		...
		float sampleDepth = (viewMatrix * vec4(texture(posTex, offset.xy).xyz, 1.0)).z; 

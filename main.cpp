...
int main()
{
	UIManager UI;
	//UI_SSAO
	if (!UI.create(900, 650, "nihilnauts", 120, UI_SOBEL, "windowFragment2"))
	{
	...
	portal1->transform.position = glm::vec3(0, 1.5, 0);
	...
	portal4->transform.position = glm::vec3(-19, 18, 0);
	...
	gun->transform.position = glm::vec3(0, 12, 0);
	//gun->mesh = portalGun;
	...
	do
	{
		...
		if (UI.isMouseDown(GLFW_MOUSE_BUTTON_1) && shootRefill > shootDelay)
		{
			...
			for (int i = 0; i < numBalls; i++)
			{
				...
				std::shared_ptr<Projectile> proj = Projectile::makeProjectile(radius, ballMesh, &UI, player->transform.position, direction, ballSpeed);
